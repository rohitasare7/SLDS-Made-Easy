...
<template>
  ...
  <div class="dashboard">
    <!-- Header -->
    <header class="">
      <div class="slds-global-header slds-grid slds-grid_align-center">
        <div class="slds-global-header__item">
          <div class="slds-text-align_center logoText">SLDS Made Easy</div>
        </div>
      </div>
    </header>
      ...
      <aside class="slds-col slds-size_1-of-7 slds-p-around_medium sidebar">
        <ul class="slds-navigation-list-vertical">
          <li class="slds-navigation-list-vertical__item" v-for="(item, index) in items" :key="index">
            <a href="#" @click="selectSection(item.id)" class="slds-navigation-list-vertical__action slds-text-link_reset br-3">
              {{ item.name }}
            </a>
          </li>
        </ul>
